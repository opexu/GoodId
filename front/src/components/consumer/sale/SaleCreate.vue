...
<div class="w-full h-fit rounded-3xl px-10 py-10 flex flex-col bg-white space-y-4">
    <div class="w-fit h-fit flex flex-row justify-start items-center space-x-8">
        ...
        <div class="w-full h-fit flex flex-col justify-center items-start space-y-2">
            <label for="item-name" class="font-bold">Адрес смартконтракта коллекции</label>
...
const IsDisabled = computed(()=>{
    if( contractAddress.value.match(/^0x[a-fA-F0-9]{40}$/g) && buyerAddress.value.match(/^0x[a-fA-F0-9]{40}$/g) && tokenId.value >= 0 && price.value ) return false
    else return true;
})
...
async function onCreateSaleOrder(){
    ...
    try{
        ...
        await MetaMaskStore.onCreateSaleOrder( contractAddress.value, buyerAddress.value, tokenId.value, price.value )
    }catch(e){
