...
<div class="w-full h-fit flex flex-col">
    ...
    <div class="w-full h-fit flex flex-row space-x-4">
        <input class="w-full h-fit px-6 py-4 border rounded-lg focus:bg-blue-50" placeholder="IMEI" type="text" :maxlength="255"
        v-model="key"
        @change="onChangeKey"
        />
...
function onChangeKey(){
    key.value.trim();
    if( key.value !== '' && value.value !== '') emit('set-field', { id: props.id, key, value });
}
